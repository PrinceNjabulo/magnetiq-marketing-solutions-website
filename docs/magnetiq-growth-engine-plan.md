...
| AI report generation | Claude API (Messages API, prompt caching for the system prompt) | Already the house model; use `claude-opus-5` (see Phase 4 - corrected from an earlier `claude-sonnet-5` note here) |
...
### Phase 4 — Claude AI report generation ⚠️ code complete, one step pending (see below)
- [x] `lib/reportPrompt.ts` — pure `buildReportPrompt()`: a system prompt establishing MagnetiQ's voice (plain English, encouraging but honest, written to a non-technical local-business owner) and a fixed, plain-English section structure (`Executive Summary`, `Search Engine Visibility (SEO)`, `Site Speed`, `Mobile Experience`, `Website Content`, `Technical Health`, `Turning Visitors Into Customers`, `Your Top Quick Wins`, `Next Steps`); the user turn passes the structured `analysisJson` findings (not raw HTML/crawl data), per the plan.
- [x] `lib/reportGeneration.ts` — calls Claude (model `claude-opus-5`, per current guidance — not `claude-sonnet-5` as originally sketched in Phase 0's stack table, corrected here), validates the returned markdown has every required section and a sane minimum length before accepting it (`validateReportMarkdown`, exported and directly unit-tested), then renders it to HTML via `marked`.
- [x] `POST /api/audit/[id]/generate` — reads `analysisJson`, generates and validates the report, stores `reportMarkdown` + `reportHtml`, sets `completedAt`, moves status to `done` (or `failed` with a specific, user-safe reason mapped from the SDK's typed error classes: rate limit, auth/config, or a generic fallback).
- [x] **New claim mechanism, not just a status flip**: added `Audit.reportStartedAt DateTime?` (migration `20260904195103_add_report_started_at`). Unlike crawling/analyzing, there's no `AuditStatus` value between `generating` and `done` to flip into as an atomic claim - flipping straight to `done` before the (paid) Claude call succeeds would show a failed attempt as complete. `reportStartedAt` is set atomically (`WHERE status = "generating" AND reportStartedAt IS NULL`) before the call, giving this cost-bearing step the same race-safety as the free/cheap earlier stages - verified under a real concurrent-request race (one call attempted, the other correctly no-op'd).
- [x] `AuditStatusPoller`'s `STAGE_ENDPOINTS` map extended with `generating: "generate"` - no other poller changes needed, confirming the Phase 3 refactor was the right shape.
- [x] Fixed the `done` status copy (`lib/audit.ts`), which previously claimed "we've emailed your full report" - false as of this phase, since Phase 5 (email delivery) doesn't exist yet. Now says "Your audit is complete," accurate for what's actually built.
- [x] Verified thoroughly *except the live model call itself* (see below): `buildReportPrompt` unit-tested (correct headers instructed, correct findings/evidence serialized, correct URL/company-name substitution); `validateReportMarkdown` unit-tested against a well-formed report and four rejection cases (empty, too short, missing title, missing a required section); `marked` HTML conversion verified on a realistic fixture. Full pipeline verified live end-to-end through `pending -> crawling -> analyzing -> generating -> failed` (no Claude credentials in this sandbox - see below) via both direct API calls and a real Playwright browser session driving the actual `/audit` form, confirming every stage transition, the new claim mechanism's concurrency safety, and the user-facing failure message all work correctly.
- [x] `tsc --noEmit`, `next lint`, `next build` all pass.
- **Exit criteria partially met**: the full pipeline runs end-to-end and produces a correctly-stored `failed` audit with a clean, specific reason when Claude is unreachable - proving every piece around the model call (claim, validation, storage, status, error mapping, UI) is correct. **What's not yet verified: an actual successful report from a real Claude API call.** This sandbox has no usable `ANTHROPIC_API_KEY` - the CLI's own session credentials aren't a substitute (different credential path, not meant for arbitrary app-level API calls) and reusing them wasn't attempted. Once a real key is set in `.env`, the exit criteria's remaining piece - **"a readable report for 3+ real test sites"** - needs a real run to confirm.
- **Still needed**: (1) a real `ANTHROPIC_API_KEY` in the deployed environment; (2) apply migration `20260904195103_add_report_started_at` to the connected Supabase project - the Supabase MCP connector was unavailable for the rest of this session after Phase 3, so unlike Phases 0-3 this migration has only been verified against a local Postgres instance, not yet applied to the live project. Do this before Phase 5 work touches `Audit` rows on Supabase.
